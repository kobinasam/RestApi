...
class logout extends REST{
    ...
    function Init(){
		 $sql=$this->sql;
		if($this->logout = true){
			$stmt = $sql->Execute($sql->Prepare("UPDATE app_users SET USR_LOGIN_STATUS = '0',USR_LASTLOGIN_DATE = ".$sql->Param('a')." WHERE USR_CODE = ".$sql->Param('b')),array($this->currentdate,$this->userid));
